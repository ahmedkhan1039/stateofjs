...
function filterByCountries(data, countries) {
    ...
    data.forEach(item => {
        ...
        if (country) {
            const trimmedItem = country.trim().toLowerCase().replace(/\./g, '');
            const isValid = countries.find(i => {
                return i.code.toLowerCase() === trimmedItem || trimmedItem.includes(i.name.toLowerCase()) ||
                    (i.possibleCombinations && i.possibleCombinations.find(i => i.toLowerCase() === trimmedItem))
            });
            if (isValid) {
...
fs.writeFile('filteredSojs18.json', JSON.stringify(filterByCountries(rawData, validCountries)) ,function(err) {
    if (err) throw err;
